...
class AngleModel {
 public:
  explicit AngleModel(std::string style_name);
  virtual ~AngleModel() = default;

  AngleModel(const AngleModel&) = delete;
  AngleModel& operator=(const AngleModel&) = delete;
  AngleModel(AngleModel&&) = delete;
  AngleModel& operator=(AngleModel&&) = delete;
  ...
  virtual index_t angle_count() const noexcept = 0;
 ...
 private:
  ...
  void require_angle_style(const input::ForceFieldSpec& forcefield) const;
  void require_angle_coeff_style(const input::AngleCoeffSpec& angle_coeff) const;

  virtual void read_settings(const input::AngleStyleSpec& angle_style) = 0;
  virtual void begin_topology(
      const system::state::HostState& host_state) = 0;
  virtual void read_coeff(const input::AngleCoeffSpec& angle_coeff) = 0;
